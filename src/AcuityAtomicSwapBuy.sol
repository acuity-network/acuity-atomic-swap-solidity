...
contract AcuityAtomicSwapBuy {
     ...
     * @dev
     */
    event LockBuy(address buyer, address seller, bytes32 hashedSecret, uint256 timeout, uint256 value, bytes32 chainIdAdapterIdAssetIdOrderId, bytes32 foreignAddress);
     ...
     * @dev
     */
    event TimeoutBuy(address buyer, bytes32 hashedSecret);

    /*
     * Called by buyer.
     */
    function lockBuy(address seller, bytes32 hashedSecret, uint256 timeout, bytes32 chainIdAdapterIdAssetIdOrderId, bytes32 foreignAddress) payable external {
        // Calculate buyLockId.
        ...
        // Log info.
        emit LockBuy(msg.sender, seller, hashedSecret, timeout, msg.value, chainIdAdapterIdAssetIdOrderId, foreignAddress);
    }
    ...
    function timeoutBuy(address seller, bytes32 hashedSecret, uint256 timeout) external {
        ...
        payable(msg.sender).transfer(value);
        // Log info.
        emit TimeoutBuy(msg.sender, hashedSecret);
    }
